...
def read_inclinometer_data(slave_id=1):
    # Parameters for reading the primary block
    # Start: 0x0000, Count: 13 registers (to cover acceleration at 0x000C)
    START_REG = 0
    COUNT = 13

    # safe_read_input_registers or modbus.read_holding_registers
    # The datasheet specifies Function Code 0x03 (Read Holding)
    raw_data = modbus.read_holding_registers(slave_id, START_REG, COUNT)

    results = {}

    if raw_data and len(raw_data) == COUNT:
        # Helper to handle 16-bit signed integers (Two's Complement)
        def to_signed(val):
            return val if val < 32768 else val - 65536

        # Angles (Registers 0, 1, 2) scaled by 100
        results["x_angle"] = to_signed(raw_data[0]) / 100.0
        results["y_angle"] = to_signed(raw_data[1]) / 100.0
        results["z_angle"] = to_signed(raw_data[2]) / 100.0

        # Acceleration (Registers 10, 11, 12) scaled by 100
        results["x_accel"] = to_signed(raw_data[10]) / 100.0
        results["y_accel"] = to_signed(raw_data[11]) / 100.0
        results["z_accel"] = to_signed(raw_data[12]) / 100.0

        # Display results
        print("X-Angle: {:.2f}°, Y-Angle: {:.2f}°".format(results["x_angle"], results["y_angle"]))

    return results


def read_noise_db(slave_id=4):
    """
    Read instantaneous noise value from SN-3002-ZS-N01 sensor.
    Register: 0000H
    Function: 03
    """

    raw = modbus.read_holding_registers(slave_id, 0, 1)

    # Expecting 2 bytes
    if isinstance(raw, list) and len(raw) == 2:

        # Convert hex bytes → integer
        value = int(raw[0], 16) * 256 + int(raw[1], 16)

        # Convert to dB
        noise_db = value / 10.0

        print("Noise Level:", noise_db, "dB")

        return noise_db

    print("Noise sensor read error")
    return None

def set_noise_sensor_baudrate(slave_id, baud):
    ...
    result = modbus.write_single_register(slave_id, 2001, baud_map[baud])

    if result == SUCCESSFUL:
        print("Baudrate changed to:", baud)
    else:
...
def set_noise_sensor_address(old_id, new_id):
    """
    Change Modbus slave address of noise sensor
    """

    result = modbus.write_single_register(old_id, 2000, new_id)

    if result == SUCCESSFUL:
        print("Slave ID changed to:", new_id)
    else:
        print("Failed to change slave ID")

    return result


# Example Usage
data = read_noise_db(3)