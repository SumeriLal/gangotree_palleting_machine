...
def decode_epm35_float(raw):
    """
    Decode EPM-35-M1 32-bit floating-point values.

    Input:
        raw : list of 4 elements returned by Modbus,
              each element is a hex byte like b'43'

    Format:
        - ASCII hex bytes
        - 32-bit IEEE-754 float
        - Word order: CDAB (word-swapped)
        - Endianness: Big-endian after reordering
    """
    ...
    try:
        # Convert ASCII hex bytes (b'43') → integer (0x43)
        ...
        print("EPM35 float decode error:", e)
        return 0.0

def read_epm35_float(slave_id, register):
    """
    Read a 32-bit float value from EPM-35-M1 using Modbus RTU.

    slave_id : Modbus slave address
    register : Starting holding register address
    """

    raw = modbus.read_holding_registers(slave_id, register, 2)

    if isinstance(raw, list) and len(raw) == 4:
        return decode_epm35_float(raw)

    return 0.0

def get_epm35_data(slave_id=7):
    """
    Read key electrical parameters from EPM-35-M1.
    """

    voltage = read_epm35_float(slave_id, 20)  # L1 Voltage
    current = read_epm35_float(slave_id, 52)  # L1 Current
    frequency = read_epm35_float(slave_id, 78)  # Frequency
    total_power = read_epm35_float(slave_id, 92)  # Power Factor

    print("Voltage L1-N : {:.2f} V".format(voltage))
    print("Current L1   : {:.3f} A".format(current))
    print("Frequency    : {:.2f} Hz".format(frequency))
    print("Power : {:.3f} kW".format(total_power))
