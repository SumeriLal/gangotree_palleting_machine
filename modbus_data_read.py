...
def get_modbus_data():
    # callback to read analog ppi_1 data registers for the 8-channel sensors reading
    ppi_registers_data = modbus.read_holding_registers(2, 1561, 8)
    ppi_register = modbus_rtu_to_decimal(ppi_registers_data)
    temp_oil_in = sensor_value(ppi_register[0])
    temp_oil_out = sensor_value(ppi_register[1])
    temp_water_ct_in = sensor_value(ppi_register[2])
    temp_water_ct_out = sensor_value(ppi_register[3])
    desiccant_ct_out = sensor_value(ppi_register[4])
    column_temp = sensor_value(ppi_register[5])
    air_out_column = sensor_value(ppi_register[6])
    desiccant_hot_out = sensor_value(ppi_register[7])

    # callback to read analog ppi_2 data registers for the 8-channel sensors reading
    ppi_registers_data_2 = modbus.read_holding_registers(3, 1561, 8)
    ppi2_register = modbus_rtu_to_decimal(ppi_registers_data_2)
    temp1 = sensor_value(ppi2_register[0])
    temp2 = sensor_value(ppi2_register[1])
    temp3 = sensor_value(ppi2_register[2])
    temp4 = sensor_value(ppi2_register[3])
    analog1 = ppi2_register[4]
    analog2 = ppi2_register[5]
    water_level = ppi2_register[6]
    analog3 = ppi2_register[7]

    data = {"temp_oil_in": temp_oil_in, "temp_oil_out": temp_oil_out, "temp_water_ct_in": temp_water_ct_in,
            "temp_water_ct_out": temp_water_ct_out, "desiccant_ct_out": desiccant_ct_out, "column_temp": column_temp,
            "air_out_column": air_out_column, "desiccant_hot_out": desiccant_hot_out, "water_level": water_level}
